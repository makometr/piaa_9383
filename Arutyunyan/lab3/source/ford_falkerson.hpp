...
static char FindMaximalEdge(const Graph& graph, char from, const std::deque<char>& neighbours,
...
inline WebPath DFSFindPath(const Graph& graph,
                              char from,
                              char to,
    ...
    char max_neighbour = FindMaximalEdge(graph, from, neighbours, visited_neighbours);
    bool found = false;

    while (!found && max_neighbour != from) {
        visited_neighbours.insert(max_neighbour);
        auto another_path = DFSFindPath(graph, max_neighbour, to, visited);
        ...
        if (!found)
            max_neighbour = FindMaximalEdge(graph, from, neighbours, visited_neighbours);
    }
...
inline WebPath DFSFindPath(const Graph& graph,
                           ...
                           char to) {
    std::set<char> visited;
    return DFSFindPath(graph, from, to, visited);
}
...
inline std::pair<Graph, int> FordFalkerson(Graph& graph, char source, char drain) {
    ...
    WebPath path = DFSFindPath(graph, source, drain);
    while (path.Size() != 0) {
        ...
        path = DFSFindPath(graph, source, drain);
    }
