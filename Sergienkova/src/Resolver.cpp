...
void Resolver::Resolve(int n)
{
	...
	// пробуем пропорционально цменьшить размер, для поиска в этом размере
	for (int i = 2; i < m_N; ++i) {
	...
	m_SumS = (m_N / 2 + 1) * (m_N / 2 + 1) + 2 * (m_N / 2) * (m_N / 2);
	...
	for (int i = m_N / 2; i > 0; --i)
		Scan(Tile(pos.x, pos.y, i));

	// удаляем массив занятых клеток
	for (int i = m_N; i < m_N; ++i) delete[] m_Data[i];
...
{
	// ограничитель
	if (m_Result.size() > 0 && m_Tiles.size() >= m_Result.size()) return true; // отсекаем ненужные переборы
	if (tile.Size >= m_N) return false;
	if (!CanAdd(tile)) return false;

	// добавляем тайл
	Add(tile);
	m_SumS += tile.Size * tile.Size;
	...
	if (m_SumS == m_N * m_N) {
		...
		for (int i = m_N / 2; i > 0; --i)
			Scan(Tile(position.x, position.y, i));
	}
