...
bool cmp_distance_for_queue (const std::tuple<char, char, float> &first, const std::tuple<char, char, float> &second)
{
...
int main()
{
    ...
    while (current_vertex != finish)
    {
        ...
        std::sort(queue.begin(), queue.end(), cmp_distance_for_queue);
        char from;
        while(!queue.empty())
        {
