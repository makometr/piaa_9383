...
def find_path(flow_graph, current, end, path='', visited=None, min_weight=sys.maxsize):
    ...
    for vertex in flow_graph[current].copy():
        euristic[vertex] = abs(ord(vertex)-ord(current))
    euristic = dict(sorted(euristic.items()))
    sorted_values = sorted(euristic.values())

    for i in sorted_values:
        for k in euristic.keys():
            if euristic[k] == i:
                sorted_euristic[k] = euristic[k]

    for vertex in sorted_euristic:
